...
Cell_income::Cell_income(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Cell_income)
{
    ui->setupUi(this);
    ui->dedit_time->setDateTime(QDateTime::currentDateTime()); // Set dedit_time to current date and time
}

Cell_income::~Cell_income()
{
    delete ui;
}

void Cell_income::setData(const QString &money, const QString &type, const QString &remarks, const QString &date)
{
    oldMoney = money;
    oldType = type;
    oldRemarks = remarks;
...
void Cell_income::on_pbtn_confirm_clicked()
{
    ...
    if (oldMoney.isEmpty() && oldType.isEmpty() && oldRemarks.isEmpty() && oldDate.isEmpty()) {
        // 执行数据库插入操作
        query.prepare("INSERT INTO income (money, typee, remarks, date, phone) "
                      "VALUES (:money, :typee, :remarks, :date, :phone)");
    } else {
        // 执行数据库更新操作
        query.prepare("UPDATE income SET money = :money, typee = :typee, remarks = :remarks, date = :date "
...
void Cell_income::on_pbtn_cancel_clicked()
{
    this->close();
}
