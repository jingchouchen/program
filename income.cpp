...
void income::setupModel()
{
    model = new QSqlQueryModel(this);
    updateTableView(); // 初始显示所有数据
}

void income::setupTableView()
{
    ui->tableView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch); // 让列自动填充宽度

    // 修改表头显示文本
    model->setHeaderData(0, Qt::Horizontal, "收入金额");
    model->setHeaderData(1, Qt::Horizontal, "收入种类");
    model->setHeaderData(2, Qt::Horizontal, "备注");
    model->setHeaderData(3, Qt::Horizontal, "收入时间");

    ui->tableView->setSelectionBehavior(QAbstractItemView::SelectRows);
    ui->tableView->setSelectionMode(QAbstractItemView::SingleSelection);
    connect(ui->tableView, &QTableView::doubleClicked, this, &income::onTableViewDoubleClicked);
}

void income::updateTableView(const QString &type, const QString &date)
{
...
void income::on_pbtn_search_clicked()
{
...
void income::on_pbtn_replace_clicked()
{
...
void income::on_pbtn_newAdd_clicked()
{
    // 创建加数据窗口
    Cell_income *m_cell_income = new Cell_income;
    m_cell_income->show();
}

void income::onTableViewDoubleClicked(const QModelIndex &index)
{
    if (!index.isValid())
        return;
    ...
    Cell_income *editWindow = new Cell_income;
    editWindow->setData(money, type, remarks, date);
    editWindow->show();
}

void income::on_pbtn_delete_clicked()
{
    QModelIndexList selectedRows = ui->tableView->selectionModel()->selectedRows();
        ...
        QSqlQuery query;
        query.prepare("DELETE FROM income WHERE money = :money AND typee = :type AND remarks = :remarks AND date = :date AND phone = :phone");
